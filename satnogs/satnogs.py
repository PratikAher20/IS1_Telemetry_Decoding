...
class Satnogs:
    def __init__(self, config):
        ...
        self.config = config
        self.db_enabled = self.config.config['database']['postgresql'][self.config.yaml_key]['enabled']
        self.norad_id = self.config.config['satnogs'][config.yaml_key]['norad_id']
        self.token = self.config.config['satnogs'][config.yaml_key]['token']
    ...
    def get_paginated_endpoint(self, max_entries=None, min_time=dt.datetime(2000, 1, 1)):
        ...
        r = requests.get(url=self.url, headers={'Authorization': f'Token {self.token}'}, verify=True)
        r.raise_for_status()
        data = r.json()
        ...
        while 'next' in r.links and (not max_entries or len(data) < max_entries) and timestamp > min_time:
            ...
            r = requests.get(url=next_page_url, headers={'Authorization': f'Token {self.token}'}, verify=True)
            r.raise_for_status()
            j = r.json()
    ...
    def get_satnogs_filename(self):
        """
        Generates satnogs .kss filename and appends it to satnogs rawfile path
        :return: Full file path with filename
        """
        now = dt.datetime.now()
        folder = self.config.config['satnogs_rawfiles'][self.config.yaml_key]['location']
        filename = now.strftime(f'{self.config.config["runtime"]["mission"]}_satnogs_%Y_%j_%H%M')
        print(folder)
    ...
    def save_satnogs_data(self):
        ...
        if self.db_enabled:
            ...
            self._logger.info("Tracking database disabled. Pulling all data from satnogs.")
            self.get_paginated_endpoint()

        # filtered = [d for d in self.data if len(d['frame']) == 510]
        # self.data = filtered
        ...
        if self.data is not None:
            ...
            with open(outfile, 'wb') as fout:
                for d in self.data:
                    try:
                        next_lines = binascii.unhexlify(d['frame'])
                        fout.write(next_lines)
                        ...
                        insert_observer_data.append((observer, str(0), str(0)))
                ...
                if self.db_enabled:
                    ...
                    cur.close()
                    self.db.conn.commit()
                self._logger.info("Created SatNOGS  file: " + outfile)
        else:
